...
for test_seed in 7 8 9
do
    for test_id_ratio in 0.25 0.5 0.75
    do
        test_n_labels=$(printf "%.0f"  $(echo  "175 * $test_id_ratio" | bc))
        echo "# of test labels: $test_n_labels"

        python -m text_classification.train_text_classifier \
            --config text_classification/configs/acid_test.yml \
            --trainer.logger.init_args.name acid_classifier_ts_${test_seed}_tr_${test_id_ratio} \
            --model.num_labels $test_n_labels \
            --model.checkpoint_path checkpoints/acid_cs/ts_${test_seed}_tr_${test_id_ratio} \
        ...
        do
            ...
            do
                ...
                python -m text_classification.train_text_classifier \
                    --config text_classification/configs/acid_val.yml \
                    --trainer.logger.init_args.name acid_classifier_ts_${test_seed}_tr_${test_id_ratio}_vs_${val_seed}_vr_${val_id_ratio} \
                    --model.num_labels $val_n_labels \
                    --model.checkpoint_path checkpoints/acid_cs/ts_${test_seed}_tr_${test_id_ratio}_vs_${val_seed}_vr_${val_id_ratio} \
