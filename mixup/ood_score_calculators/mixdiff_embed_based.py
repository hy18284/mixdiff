...
class MixDiffEmbedBasedMixin(nn.Module):
    def __init__(
        self,
        batch_size: int,
    ...
    def process_images(
        ...
        logits, embeds, class_embeds = self._process_images(images, return_embeds=True)
        kwargs = {
            'logits': logits,
            'embeds': embeds,
    ...
    def _process_images(
        ...
        for split_images in images_list:
            outputs = self.backbone.process_images(
                split_images, 
                return_embeds=return_embeds,
            )
            if return_embeds:
                split_logits, split_embeds, split_class_embeds = outputs
            else:
                split_logits = outputs

            logits_list.append(split_logits) 
            if return_embeds:
                embeds_list.append(split_embeds)
                class_embeds_list.append(split_class_embeds)
        
        logits = torch.cat(logits_list, dim=0).float()
        if return_embeds:
            embeds = torch.cat(embeds_list, dim=0)
            class_embeds = torch.cat(class_embeds_list, dim=0)
        if self.intermediate_state == 'softmax':
        ...
        if return_embeds:
            return logits, embeds, class_embeds
        else:
            return logits
