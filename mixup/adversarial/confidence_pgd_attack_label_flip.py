# Came from https://github.com/jfc43/robust-ood-detection/blob/master/utils/confidence_pgd_attack.py
...
class OELoss(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        return -(x.mean(1) - torch.logsumexp(x, dim=1)).sum()


class LabelFlip(nn.Module):
    def __init__(self):
        super().__init__()
    
    def forward(self, x: torch.Tensor, labels, *args, **kwargs):
        y = x.clone().detach()
        for i, label in enumerate(labels):
            y[i][label] = float('-inf')
        max_labels = torch.argmax(y, dim=1)
        loss = F.cross_entropy(x, max_labels, reduction='sum')
        return loss
...
class ConfidenceLinfPGDAttack:
    ...
    def __init__(
        ...
        if label_flip:
            self.loss_func = LabelFlip()

        self.clip_min = clip_min
        self.clip_max = clip_max
        self.num_classes = num_classes

    def perturb(self, x, labels = None):
        """
